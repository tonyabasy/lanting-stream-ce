...
import com.lanting.admin.module.table.dto.ParseTableRequest;
...
public class TableController {
    ...
    @Operation(summary = "反序列化：DDL 文本 → 结构化表单数据")
    @PostMapping("/utils/deserialize")
    public Result<FlinkTable> stringToCreateTable(@RequestBody @Valid ParseTableRequest request) {
        return Result.success(FlinkSqlParser.parseCreateTable(request.getDdl()));
    }

    @Operation(summary = "序列化：表单数据 → DDL 文本")
    @PostMapping("/utils/serialize")
    public Result<String> createTableToString(@RequestBody FlinkTable form) {
        return Result.success(FlinkSqlParser.createTableToString(form));
    }
