...
### 文件树
- [x] ① `tree(String parentPath, String sort)` — 文件树查询

### 读取
- [x] ② `content(Long fileId)` — 读取磁盘当前文件内容

### 写入
- [x] ③ `create(CreateFileDTO dto)` — 创建空文件
- [x] ④ `save(SaveFileDTO dto)` — 自动保存到磁盘
- [x] ⑤ `createFolder(CreateFolderDTO dto)` — 创建文件夹
- [x] ⑥ `rename(RenameDTO dto)` — 重命名文件或文件夹

### 删除与回收站
- [x] ⑦ `delete(Long fileId, boolean force)` — 删除文件/文件夹（软删除）
- [x] ⑧ `trash(String parentPath)` — 回收站文件树
- [x] ⑨ `purge(Long fileId)` — 从回收站彻底删除
- [x] ⑩ `restore(RestoreFileDTO dto)` — 恢复已删除文件/文件夹

### 提交
- [x] ⑪ `commit(CommitFileDTO dto)` — 提交文件到 Git

### 历史与对比
- [x] ⑫ `history(HistoryPageQuery query)` — 查询 Git 文件历史（游标分页）
- [x] ⑬ `diff(Long fileId, String from, String to)` — 计算两 commit 间 unified diff

### 回滚
- [x] ⑭ `revert(RevertFileDTO dto)` — 文件级软回滚
...
### 基础设施
- [x] ⑰ `withWorkspaceLock(Supplier<T>)` — Git 写锁包装，串行化 add/commit/tag
- [-] ⑱ `resolvePathByFileId(Long fileId)` — 通过 fileId 解析路径（含已删除）
- [x] ⑲ `validatePath(String path)` — 路径合法性校验（第一道防线）
- [x] ⑳ `ensureInsideWorkspace(Path target, Path root)` — 工作空间边界校验（第二道防线）
- [x] ㉑ `validateFileType(Path path)` — 文件扩展名白名单校验
- [-] ㉒ `relativePath(Path path, Path root)` — 计算相对路径

### 树与排序
- [x] ㉓ `sortTreeNodes(List<FileTreeNode>, String sort)` — 排序树节点
