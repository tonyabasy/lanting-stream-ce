...
- 现状的 FileTabs / CodeEditor 组装逻辑**原地保留**（Tabs + 锁管理不变）
- 只把内容区从「固定 CodeEditor」改为「按 fileType 路由」
- `readonly` 已实现（banner 已删，只读靠编辑器自行表现）

### 2. `components/TableEditor/index.tsx`（新）— DDL 双模态容器（模块入口）

- 参考 table-implementation-plan CP3.2：`fileId` / `readonly` / `content` props
- `mode: 'form' | 'text'` state，默认 `'form'`
- Segmented `[表单 | 文本]` 切换
- 表单→文本：`serializeFormToTable(formData)` 写入文本（单向，CP4 再做双向）
- 文本模式复用 CodeEditor

### 3. `components/TableEditor/tableSerializer.ts`（新）

- `serializeFormToTable(data: TableFormData): string`
- 输出 `CREATE TABLE ... (字段) [PARTITIONED BY (...)] WITH (...)`（参考 CP3.4）

### 4. `components/TableEditor/TableFormEditor.tsx`（新）

- 四段式：基本信息 / 字段定义 / 分区 / 连接器属性（参考 CP3.3）
- `TableFormData { tableName, connector, fields[], partitionFields[], properties[] }`
- 「确定」→ serializeFormToTable → `saveFile(fileId, content)`
- `readonly=true` 字段禁用
...
| Step | 内容 | 产出 |
|------|------|------|
| 1 | EditorPanel 内容区改按 fileType 路由（`'ddl'` → TableEditor，其他 → CodeEditor） | EditorPanel.tsx |
| 2 | TableEditor 容器（Segmented + mode state） | components/TableEditor/index.tsx |
| 3 | tableSerializer | components/TableEditor/tableSerializer.ts |
| 4 | TableFormEditor 四段表单 | components/TableEditor/TableFormEditor.tsx |
| 5 | tsc + 手动验证 | — |
