...
> 所有样式值从 `useModel('theme')` 取，禁止在组件里 hardcode 任何颜色、字号、间距、圆角值。
...
interface LantingToken extends NonNullable<ThemeConfig['token']> {
  ...
  // ── 自定义背景 ──────────────────────────────────────
  colorBgSubtle:    string;  // 侧边栏、stat 块、代码行号区
  colorBgMuted:     string;  // 更深一层的背景，分隔块

  // ── 自定义文字 ──────────────────────────────────────
  colorNeutral:     string;  // 中性状态色，如已停止的作业点

  // ── 字体 ────────────────────────────────────────────
  fontFamilyCode:   string;  // 代码、数值专用字体

  // ── 字号 ────────────────────────────────────────────
  fontSizeCaption:  number;  // 12  badge、行号、辅助文字
  fontSizeBody:     number;  // 14  正文
  fontSizeTitle:    number;  // 16  卡片标题
  fontSizeHeading:  number;  // 20  页面标题

  // ── 字重 ────────────────────────────────────────────
  fontWeightRegular: number; // 400  正文
  fontWeightMedium:  number; // 500  标题、强调
...
间距只从以下八个档位取值，禁止使用其他数值：

| Token | 值 | 典型用途 |
|-------|-----|---------|
| `spacingXS` | 4 | 图标与文字间距、紧凑行内间距 |
| `spacingSM` | 8 | 按钮内边距（垂直）、列表行间距 |
| `spacingMD` | 12 | 卡片内边距（小）、表单项间距 |
| `spacingLG` | 16 | 卡片内边距（标准）、section 间距 |
| `spacingXL` | 20 | 页面内容区 padding（垂直） |
| `spacing2XL` | 24 | 页面内容区 padding（水平） |
| `spacing3XL` | 32 | 页面区块间距 |
| `spacing4XL` | 48 | 页面顶部留白 |
...
✗ padding: '12px 16px'          → 用 token.spacingMD / token.spacingLG
✗ borderRadius: 6               → 用 token.borderRadius
✗ fontWeight: 600               → 只允许 400 / 500
✗ margin: 10px                  → 不在间距档位内，禁止使用
