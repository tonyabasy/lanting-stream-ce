...
```
1. DB UPDATE：将目标节点及子节点的 deleted_at 置为当前时间戳
2. 磁盘删除
3. 磁盘失败 → 抛异常，文件已标记为删除但磁盘残留，reconcile 时记录为 stale
```

先更新 DB：索引标记为已删除后即使磁盘删失败，用户视角文件已不存在（`tree()` 过滤 deleted_at = 0），残留文件由 reconcile 清理。
