...
- `save`：自动保存到磁盘，要求持锁
- `createFolder`：创建目录并写入 `.gitkeep` 后提交
...
| 路径为 `.gitkeep` 或包含 `.gitkeep` | 返回 `PATH_ILLEGAL`（系统占位文件，禁止操作） |
...
| 删除 `.gitkeep` | 在 `validatePath` 阶段拒绝 |
...
| `from == to` | 返回空字符串，不抛异常 |
| tree 根层级不显示 `.gitkeep` | `.gitkeep` 被过滤，文件树中不可见 |
| `rollbackCheck` 结果不含 `.gitkeep` | tag 中的 `.gitkeep` 不进入锁定检查清单 |
...
| `tree` 根层级不显示 `.gitkeep`；`rollbackCheck` 结果不含 `.gitkeep` | `.gitkeep` 三处过滤修复 | 🟡 |
